...
const options = {
    ...
    headers: {
        ...
        Authorization: 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIwYmY4NDQzNmE2ODJmZmEwYWZiNGY0Y2EyMDI3NT' +
                'I1MCIsIm5iZiI6MTcyOTU5NTQyOS44MzkyMSwic3ViIjoiNjcwZjAwZjI3YTIyYmYzNjA3ZjI2ZWQy' +
                'Iiwic2NvcGVzIjpbImFwaV9yZWFkIl0sInZlcnNpb24iOjF9.faXY55W6bg-SOjoTgaCt5E28vCwmq' +
                'Ir1MYSVngO4qZU' // 여기에 본인의 API 키 입력
    }
};
...
const showMovie = (movies) => {
    ...
    movies.forEach(movie => {
        ...
        movieCard.innerHTML = `<img src="https://image.tmdb.org/t/p/w500${movie.poster_path}" alt="${movie.title}">
      <h3>${movie.title}</h3>
...
removebookMark.addEventListener('click', () => {
    ...
    const bookmarks = localStorage.getItem('bookmarks') ? JSON.parse(localStorage.getItem('bookmarks')) : []; // ID에 해당하는 북마크 제거
    
     if (bookmarks.some(movie => movie.id === removeMovieId)) {
        removeBookmark(removeMovieId);
        alert('북마크가 삭제되었습니다!');
